...
metaData.ecoCode.climate = {'Cfb'}; % Cfb Temperate oceanic climate; coldest month averaging above 0 °C, all months with average temperatures below 22 °C...
...
metaData.ecoCode.gender  = {'Dg'}; % Dg gender determined by genetics
metaData.ecoCode.reprod  = {'O'}; % O homogamy (no alternating generations, fertilisation obligatory)

metaData.T_typical  = C2K(20); % K, typical body temp
metaData.data_0     = {'ah';'ab';'Lh'; 'Lb'; 'Lp'; 'Li'; 'Wwh'; 'Wwb'; 'Wwi';'Wdh';'Wdb';'Ri'}; 
metaData.data_1     = {'t-L_T';'L-Ww';'L-Ri';'t-Wd';'T-JO';'Wd-JO_T'};

metaData.COMPLETE = ?; % using criteria of LikaKear2011

metaData.author   = {'Marine Uguen', 'Sylvie Gaudron'};  
metaData.date_subm = [2021 05 11];  
metaData.email    = {'marine.uguen.etu@univ-lille.fr'}; 
...
% weight at puberty
%data.Wdp =  ;     units.Wdp = 'g';    label.Wdp = 'dry weight at puberty';  bibkey.Wdp = '';
  %comment.Wdp  = '';

  
% REPRODUCTION 
data.Ri = 2*4e6/365 ;     units.Ri = '#/d';    label.Ri = 'maximal reproduction rate';  bibkey.Ri = 'Sprung1983';
  comment.Ri  = 'intertidal female max ; 2 spawning periods in France';
...
% WEIGHT - LENGTH 
%pareil ici met si c'est shell length au lieu de shell length
...
data.O.LW = [ ... % shell length (cm), wet weight (g)
4.951	2.817;
4.515	2.354;
4.58	2.751;
4.374	3.382;
...
3.896	1.9358;
3.33	1.42371];
units.O.LW = {'cm', 'g'}; label.O.LW = {'length', 'wet weight'};  
bibkey.O.LW = 'Uguen2021'; comment.O.LW = 'Pointe aux Oies site';
